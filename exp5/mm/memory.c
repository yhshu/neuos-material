...
// Linux 0.11 使用最简单的方法，即内存字节位图来管理物理页的状态, 声明时全部填零
// 每一个字节表示这个物理页面被共享次数
...
void free_page(unsigned long addr) {
    ...
    mem_map[addr] = 0;
    panic("Trying to free the free page!");  // 不应该出现这种情况 因而引发panic
}
...
// 则尝试共享页面,如果仍然失败,则尝试取一个新的页面(不在内存中的),再失败则报错OOM
// 参数 error_code, 错误号; address (线性地址)异常地址
void do_no_page(unsigned long error_code, unsigned long address) {
    printk("Page Fault at [%x]\n", address);
    unsigned long page;                    // 申请的页面

    // ** 实验 5 Step 2 ** //
    // ** 请在此处键入你的代码 ** //
                                            // 取异常地址所在页的首地址，即置页偏移为0
                                            // 调用get_free_page函数申请新的物理页
                                            // 如果申请失败，则调用oom函数报错
                                            // 调用put_page函数将申请到的物理页映射到异常地址所在页面首地址处
                                            // 如果申请失败，释放刚申请的内存页 page，调用oom报错
